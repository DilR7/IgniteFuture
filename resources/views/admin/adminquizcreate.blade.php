...
@section('content')
    ...
    <form action="{{ route('adminquiz.post') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
            <label for="img" class="block text-sm font-medium">Quiz Image</label>
            <input type="file" name="img" id="img" class="w-full border-gray-300 rounded-md"
                required>{{ old('img') }}</input>
            @error('img')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div id="questions-container">
            ...
            <button type="button" class="bg-green-600 text-white px-3 py-2 rounded mb-4" onclick="addQuestion()">+ Add
                Question</button>
        </div>
    ...
    <div class="question-block mb-6 border p-4 rounded-md">
       ...
       <input type="number" name="questions[${questionIndex}][point]" class=" w-full border-gray-300 rounded-md mb-2" value="10" readonly style="background-color: #f3f4f6; color: #6b7280; border-color: #d1d5db;">
