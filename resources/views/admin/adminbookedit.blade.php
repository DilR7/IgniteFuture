...
@section('content')
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4">
        <form action="{{ route('adminbook.update', $book->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700">Book Name</label>
                <input type="text" name="name" id="name"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('name', $book->name) }}"
                    required>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="file" name="content" id="content"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" rows="3"></input>
            </div>

            <div class="mb-4">
                <label for="img" class="block text-sm font-medium text-gray-700">Book Cover (Optional)</label>
            ...
            <div class="mb-4">
                ...
                <select name="module_id" id="module_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                    ...
                    @foreach ($modules as $module)
                        <option value="{{ $module->id }}" {{ $module->id == $book->module_id ? 'selected' : '' }}>
                            {{ $module->name }}
                        </option>
                    @endforeach
                </select>
